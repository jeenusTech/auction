        ...
        <div class="col-12">
          <div class="card card-navy border-dark">
            <div class="card-header">
              <h3 class="card-title">E-Auctions</h3>
              ...
              <table id="categorytable" class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        {{-- <form id="status{{ $auction->id }}" action="{{route('auction_status',$auction->id)}}" method="post" style="display: inline;">
                            @csrf
                            <label id="switch">
                        ...
                        @endphp
                        @if($today < \Carbon::createFromTimestamp(strtotime($auction->start_date.$auction->start_time)))
                        <span class=" badge bg-primary">Upcoming</span>
                        @elseif($auction->status==1)
                        <span class=" badge bg-success">Latest</span>
                        @elseif($today > \Carbon::createFromTimestamp(strtotime($auction->end_date.$auction->end_time)))
    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
          <div class="card card-navy border-dark">
            <div class="card-header">
              <h3 class="card-title">Floor Auctions</h3>
              <div class="card-tools">
                <button type="button" class="btn btn-tool" data-card-widget="collapse">
                  <i class="fas fa-minus"></i>
                </button>
            ...
            <!-- /.card-header -->
            <div class="card-body">
              <table id="categorytable" class="categorytable table table-bordered table-hover">
                <thead>
                <tr>
                  <th>Auction no.</th>
                  <th>Title</th>
                ...
                <tbody>
                    ...
                    @foreach ($floorauctions as $auction)
                    <tr>
                      <td>{{$auction->auction_number}}</td>
                      <td>{{$auction->title}}</td>
                      {{-- <td>{{$auction->auction_category->cat_name}}</td> --}}
